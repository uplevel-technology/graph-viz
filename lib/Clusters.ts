...
export class Clusters {
  ...
  public data: GraphVizCluster[]

  private readonly geometry: THREE.BufferGeometry
  private readonly material: THREE.ShaderMaterial

  constructor(clusters: GraphVizCluster[]) {
    const numClusters = size(clusters)

    this.geometry = new THREE.BufferGeometry()
    this.geometry.addAttribute(
      'position',
      new THREE.BufferAttribute(new Float32Array(numClusters * 3), 3),
    )
    this.geometry.addAttribute(
      'absoluteSize',
      new THREE.BufferAttribute(new Float32Array(numClusters * 1), 1),
    )
    this.geometry.addAttribute(
      'scale',
      new THREE.BufferAttribute(new Float32Array(numClusters * 1), 1),
